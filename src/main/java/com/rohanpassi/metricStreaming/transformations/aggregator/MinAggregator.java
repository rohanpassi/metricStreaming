...
@Component
public class MinAggregator implements MetricAggregator {
    ...
    public List<Metric> aggregate(List<Metric> metrics) {
        ...
        Optional<Metric> minMetric = metrics.stream()
                .min((m1, m2) -> Double.compare(m1.getValue(), m2.getValue()));

        List<Metric> result = new ArrayList<>();
        minMetric.ifPresent(result::add);
        return result;
    }
}